...
| AP-12 | Using `#include <regex>` or other heavyweight headers without a direct use | The project compiles with `-Wall -Wextra -Wpedantic`; unused includes produce warnings. `<regex>` is the canonical example of a heavyweight header with no use in this codebase |
